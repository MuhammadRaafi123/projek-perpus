...
function incrementStock(kode_buku) {
  fetch("/api/books/update-stock", {
    ...
    body: JSON.stringify({ kode_buku, action: "increment" }),
  }).catch(console.error);
}

export default function PeminjamanPage() {
  const [peminjaman, setPeminjaman] = useState([]);
  const [currentUser, setCurrentUser] = useState(null); // State untuk user yang login
  ...
  const currentUsername = currentUser?.username || currentUser?.nama_lengkap;
  const userPeminjaman = peminjaman.filter((item) => (item.username === currentUsername) || (item.user === currentUsername));

  // 3. FILTER STATUS & SORTING: Gunakan data userPeminjaman, bukan semua peminjaman
  const filtered = userPeminjaman
  ...
  // 4. Hitung badge status HANYA untuk user yang login
  const countByStatus = (status) => userPeminjaman.filter((i) => i.status === status).length;
  ...
  const confirmKembalikan = () => {
    ...
    if (saved) {
      ...
      // Update data di array keseluruhan
  ...
  const confirmHapus = () => {
    const saved = localStorage.getItem("peminjaman");
    if (saved) {
      // Hapus data dari array keseluruhan
  ...
  const handleReset = () => {
    ...
    setTimeout(() => {
      ...
      // 5. RESET AMAN: Hanya hapus data milik user yang sedang login
      const otherUsersData = allData.filter((item) => (item.username !== currentUsername) && (item.user !== currentUsername));
      const myData = allData.filter((item) => (item.username === currentUsername) || (item.user === currentUsername));
      ...
      // Timpa localStorage hanya dengan data user lain
      localStorage.setItem("peminjaman", JSON.stringify(otherUsersData));

      // Opsional: Lakukan filter yang sama untuk "pengembalian" dan "riwayat" jika diperlukan

      loadPeminjaman();
      setFilterStatus("semua");
      setResetLoading(false);
      setShowResetModal(false);
      alert("✅ Data peminjaman kamu berhasil direset!");
    }, 800);
  };
    ...
    <div className="flex bg-gray-100 min-h-screen">
      <aside className="w-64 bg-gradient-to-b from-gray-700 to-gray-800 text-white fixed h-full shadow-2xl flex flex-col">
        ...
        <nav className="flex-1 py-6 space-y-2">
          <Link href="/home" className="flex items-center gap-4 px-6 py-4 hover:bg-gray-700 transition"><Home size={22} /> Beranda</Link>
      ...
      <main className="ml-64 p-10 w-full">
        <div className="bg-white p-6 rounded-xl shadow mb-6 border-l-4 border-yellow-500 flex items-center justify-between">
          ...
          {userPeminjaman.length > 0 && (
            ...
            >
              <RefreshCw size={16} /> Reset Data Kamu
            </button>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
          {[
            { label: "Semua", value: userPeminjaman.length, status: "semua" },
          ...
          <div className="bg-white rounded-xl w-full max-w-md shadow-lg overflow-hidden">
            <div className={`p-6 border-b ${confirmAction === "hapus" ? "bg-red-50 border-red-200" : "bg-green-50 border-green-200"}`}>
              ...
              <p className="text-gray-500 mt-1 text-xs">
                {confirmAction === "hapus" ? "Data akan dihapus permanen dari daftarmu." : "Pastikan buku sudah benar-benar dikembalikan."}
              </p>
            </div>
          ...
          <div className="bg-white rounded-xl w-full max-w-md shadow-lg overflow-hidden">
            <div className="bg-red-50 border-b border-red-200 p-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h2 className="text-xl font-bold text-red-700">Reset Data Kamu</h2>
              </div>
              <p className="text-gray-600 text-sm leading-relaxed">
                Semua data peminjamanmu akan <strong>dihapus permanen</strong>. Stok buku yang sedang kamu pinjam akan dikembalikan otomatis. (Data user lain tidak akan terpengaruh).
              </p>
            </div>
            ...
            <div className="px-6 py-4 bg-yellow-50 border-b border-yellow-100">
              ...
              <ul className="text-xs text-yellow-700 space-y-1">
                <li>✅ Peminjamanmu ({userPeminjaman.length} data)</li>
                <li>✅ Stok buku yang kamu pinjam dikembalikan</li>
              </ul>
            </div>
