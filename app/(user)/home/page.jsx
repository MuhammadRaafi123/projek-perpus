...
function getDBStock(booksFromDB) {
  const stockMap = {};
  booksFromDB.forEach((b) => {
    stockMap[b.kode_buku] = b.stok_tersedia ?? b.stok_total ?? 0;
  });
  return stockMap;
}
...
function decrementStock(kode_buku, currentStockMap) {
  ...
  return { ...currentStockMap, [kode_buku]: Math.max(0, current - 1) };
}
...
export default function HomePage() {
  ...
  useEffect(() => {
    fetch("/api/books")
      ...
      .then((data) => {
        ...
        setBooks(normalised);
        const merged = getDBStock(normalised);
        setBookStock(merged);
      })
      .catch(() => {})
  ...
  useEffect(() => {
    try {
      ...
      const currentUsername = user?.username || user?.nama_lengkap;
      ...
      if (savedWishlist) {
        ...
        setWishlist(parsedWishlist.filter(b => b.username === currentUsername));
      }
      
      if (savedLoans) {
        const data = JSON.parse(savedLoans);
        const userLoans = data.filter(item => (item.username === currentUsername) || (item.user === currentUsername));
        setLoans(userLoans);
        
        const active = userLoans.filter((i) => ["dipinjam", "disetujui", "pending"].includes(i.status)).length;
        setActiveLoans(active);
        
        const dipinjam = userLoans.filter((i) => i.status === "dipinjam").length;
        setMyLoans(dipinjam);
      }
  ...
  const toggleWishlist = (book) => {
    ...
    const allSaved = JSON.parse(localStorage.getItem("wishlist") || "[]");
    
    const existsIndex = allSaved.findIndex((b) => b.kode_buku === book.kode_buku && b.username === currentUsername);
    let updatedAll;
    
    if (existsIndex >= 0) {
      updatedAll = allSaved.filter((_, i) => i !== existsIndex);
    } else {
      updatedAll = [...allSaved, {
    ...
    setWishlist(updatedAll.filter(b => b.username === currentUsername));
  };
  ...
  const submitPinjam = () => {
    ...
    const currentUsername = currentUser?.username || currentUser?.nama_lengkap || "Guest";
    const newLoan = {
      id: `loan_${Date.now()}`,
    ...
    fetch("/api/books/update-stock", {
      ...
      body: JSON.stringify({ kode_buku: selectedBook.kode_buku, action: "decrement" }),
    }).catch(console.error);
    ...
    <div className="flex bg-gray-100 min-h-screen">
      ...
      <aside className="w-64 bg-gradient-to-b from-gray-700 to-gray-800 text-white fixed h-full shadow-2xl flex flex-col">
        ...
        <nav className="flex-1 py-6 space-y-2">
          <Link href="/home" className="flex items-center gap-4 px-6 py-4 bg-yellow-600/20 border-r-4 border-yellow-500 text-yellow-400">
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white w-full max-w-md p-6 rounded-xl shadow-lg">
            ...
            <div className="mb-3">
              ...
              <input type="text" className="w-full px-3 py-2 border rounded-lg bg-gray-100 text-gray-600 cursor-not-allowed"
                value={currentUser?.username || currentUser?.nama_lengkap || "Guest"} readOnly />
            </div>
            <div className="mb-5">
