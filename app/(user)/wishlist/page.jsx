...
export default function WishlistPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      // Filter here so user only sees their wishlist items
      const userWishlist = fixed.filter(b => b.username === currentUsername);
      setWishlist(userWishlist);
      localStorage.setItem("wishlist", JSON.stringify(fixed)); // save the un-filtered fixed structure
    }
  }, []);
  ...
  const removeFromWishlist = (key) => {
    ...
    // Find the item to remove globally
    const updatedAll = allSaved.filter((b, i) => {
      // Remove if it matches the key AND belongs to the current user
      if (getBookKey(b, i) === key && b.username === currentUsername) {
        return false;
      }
      return true;
    });
...
<div className="fixed inset-0 bg-black/40 flex items-center justify-center">
  <div className="bg-white p-6 w-full max-w-md rounded-xl shadow-lg">
    ...
    <div className="mb-4">
      ...
      <input
        type="text"
        className="w-full border px-3 py-2 rounded-lg mt-1 bg-gray-100 text-gray-600 cursor-not-allowed"
        value={currentUser?.username || currentUser?.nama_lengkap || "Guest"}
        readOnly
      />
    </div>
