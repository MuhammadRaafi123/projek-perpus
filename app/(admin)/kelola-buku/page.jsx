...
export default function KelolaBukuPage() {
    ...
    <div className="min-h-screen flex bg-gray-100">
      ...
      <aside className="w-64 bg-yellow-800 text-white flex-col p-6 shadow-xl hidden md:flex">
        ...
        <nav className="flex flex-col space-y-4">
          <button onClick={() => router.push("/dashboard")} className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-yellow-700 transition">
  ...
  <div className="relative w-full max-w-md">
    <Search className="absolute left-3 top-3 text-gray-800 w-5 h-5" />
    <input
      value={search}
      onChange={(e) => setSearch(e.target.value)}
      placeholder="Cari judul, pengarang..."
      className="w-full pl-10 pr-3 py-2.5 rounded-xl border bg-white shadow focus:outline-none focus:ring-2 focus:ring-yellow-400 placeholder:text-gray-700 placeholder:font-medium"
    />
  </div>
